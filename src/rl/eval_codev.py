...
def kill_process_tree(pid):
    try:
        ...
        print(f"Kill process tree failed: {e}", file=sys.stderr)
...
def verify_one_sample_wrapper(args):
    ...
    queue = Queue()
    process = Process(target=lambda: queue.put(verify_one_sample(*args)))
    process.daemon = True  # 设为 daemon，主进程退出时自动杀
    process.start()
    
    try:
        result = queue.get(timeout=30)
        process.join(timeout=2)  # 给一点时间正常退出
        return result
    except Empty:
        print("Function timed out!")
        if process.is_alive():
            kill_process_tree(process.pid)
            process.join(timeout=5)  # 最多再等 5 秒
            if process.is_alive():
                print("Warning: Process still alive after kill!")
        return {"correct": False, "timeout": True}
    finally:
        # 保险起见
        if process.is_alive():
