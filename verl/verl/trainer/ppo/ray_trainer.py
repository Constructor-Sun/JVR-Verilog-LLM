            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        # ==================== DAPO / Mixed Thinking  ====================
                        if self.config.reward_model.get("mixed_thinking", False):
                            gamma = self.config.reward_model.get("dapo_gamma", 0.3)
                            tau = self.config.reward_model.get("dapo_tau", 0.35)
                            
                            from collections import defaultdict
                            # offload to CPU
                            scores_cpu = reward_tensor.sum(dim=-1).cpu().numpy()
                            batch_size = len(batch)
                            
                            # 初始化日志标签，长度必须对齐 batch_size 以防报错
                            distill_success_flags = np.zeros(batch_size, dtype=np.float32)
                            
                            # 1. group by pair_id ( extra_info["index"])
                            pair_groups = defaultdict(lambda: {"think_indices": [], "nothink_indices": [], "t_scores": []})
                            for i in range(batch_size):
                                info = batch.non_tensor_batch.get("extra_info", [{}])[i]
                                p_id = info.get("index") 
                                mode = info.get("mode") # "think" or "no_think"
                                
                                if mode == "think":
                                    pair_groups[p_id]["think_indices"].append(i)
                                    pair_groups[p_id]["t_scores"].append(scores_cpu[i])
                                elif mode == "no_think":
                                    pair_groups[p_id]["nothink_indices"].append(i)

                            # 2. use pair-wise logic to modify rewards
                            expected_reward = self.config.reward_model.get("expected_reward", 0.5)
                            for p_id, bucket in pair_groups.items():
                                t_indices = bucket["think_indices"]
                                nt_indices = bucket["nothink_indices"]
                                # t_scores = bucket["t_scores"]

                                t_scores = [scores_cpu[i] for i in t_indices]
                                if not t_scores or (sum(t_scores) / len(t_scores)) < tau:
                                    continue

                                # avg_t_score = sum(t_scores) / len(t_scores)

                                successful_nt_in_group = [i for i in nt_indices if scores_cpu[i] >= expected_reward]

                                if len(successful_nt_in_group) > 0:
                                    for idx in (nt_indices + t_indices):
                                        if scores_cpu[idx] >= expected_reward:
                                            # avoid repeat bonus
                                            if distill_success_flags[idx] == 0:
                                                existing_reward_mask = (reward_tensor[idx, :] != 0.0)
                                                actual_pos = torch.where(existing_reward_mask)[0][-1]
                                                # m = batch.batch["attention_mask"][idx]
                                                # last_token_pos = min(m.sum() - 1, reward_tensor.size(1) - 1)
                                                # if existing_reward_mask.any():
                                                # print("before reward_tensor: ", reward_tensor[idx, :last_token_pos+1])
                                                reward_tensor[idx, actual_pos] = expected_reward + gamma
                                                # print("after reward_tensor: ", reward_tensor[idx, :last_token_pos+1])
                                                distill_success_flags[idx] = 1.0
                                                # else:
                                                #     # 默认right padding
                                                #     print("errors when calculating padding position")
                                                #     print("before reward_tensor: ", reward_tensor[idx, :last_token_pos+1])
                                                #     reward_tensor[idx, last_token_pos] = expected_reward + gamma
                                                #     print("after reward_tensor: ", reward_tensor[idx, :last_token_pos+1])
                                                #     distill_success_flags[idx] = 1.0

                            # 3. add distill_success_flags to wandb logging
                            reward_extra_infos_dict["critic/distill_success"] = distill_success_flags.tolist()
                        # ==================== DAPO / Mixed Thinking ====================
                        ...
                        # ==================== debugging ====================
                        log_freq = self.config.reward_model.get("log_freq", 1)
                        if self.config.reward_model.get("mixed_thinking", False) and self.global_steps % log_freq == 0:
                            import sys
                            
                            final_scores = reward_tensor.sum(dim=-1).cpu().numpy()
                            modes = batch.non_tensor_batch.get("extra_info", [{}])
                            
                            stats = {
                                "think_base": 0, "think_distilled": 0, 
                                "nothink_base": 0, "nothink_distilled": 0, 
                                "failure": 0
                            }
                            
                            output_buffer = []
                            output_buffer.append(f"\n{'='*60}\nDEBUG: Step {self.global_steps} | DAPO Reward Distribution\n{'-'*60}")
                            output_buffer.append(f"{'Index':<8} | {'Mode':<10} | {'Reward':<10} | {'Status'}")
                            output_buffer.append("-" * 60)

                            for i in range(len(final_scores)):
                                info = modes[i] if i < len(modes) else {}
                                mode = info.get("mode", "unknown")
                                score = final_scores[i]
                                
                                is_correct = score >= 0.69
                                is_bonus = score >= 0.99
                                
                                # print top-15 samples
                                if i < 15:
                                    status_str = "★ [Distilled]" if is_bonus else ("✔ [Correct]" if is_correct else "✘ [Failed]")
                                    output_buffer.append(f"{i:<8} | {mode:<10} | {score:<10.2f} | {status_str}")
                                
                                if mode == "think":
                                    if is_bonus: stats["think_distilled"] += 1
                                    elif is_correct: stats["think_base"] += 1
                                    else: stats["failure"] += 1
                                elif mode == "no_think":
                                    if is_bonus: stats["nothink_distilled"] += 1
                                    elif is_correct: stats["nothink_base"] += 1
                                    else: stats["failure"] += 1
                                else:
                                    stats["failure"] += 1

                            summary = (
                                f"{'-'*60}\nBatch Summary (Size: {len(final_scores)}):\n"
                                f" - Think Mode:   Correct Total = {stats['think_base'] + stats['think_distilled']} "
                                f"(Base: {stats['think_base']}, Bonus: {stats['think_distilled']})\n"
                                f" - No-Think Mode: Correct Total = {stats['nothink_base'] + stats['nothink_distilled']} "
                                f"(Base: {stats['nothink_base']}, Bonus: {stats['nothink_distilled']})\n"
                                f" - Total Failures: {stats['failure']}\n"
                                f"{'='*60}\n"
                            )
