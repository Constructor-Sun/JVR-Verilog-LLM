...
# Self-defined pair random sampler
class StatefulPairRandomSampler(RandomSampler):
    def __init__(self, data_source, **kwargs):
        assert len(data_source) % 2 == 0, "Dataset size must be even for paired sampling."
        super().__init__(data_source=data_source, **kwargs)
        self.num_pairs = len(data_source) // 2

    def __iter__(self):
        # we first create a shuffled index with length 'num_pairs' for pair shuffling
        pair_generator = torch.randperm(self.num_pairs, generator=self.generator).tolist()
        
        # 2. 按照打乱的“对”索引，依次吐出 (T, NT)
        for p_idx in pair_generator:
            yield p_idx * 2
            yield p_idx * 2 + 1
...
def create_rl_sampler(data_config, dataset):
    ...
    # If shuffling is enabled in the data configuration, create a random sampler.
        ...
        # This defines how to sample paired think / nothink data
        print("mixed_thinking: ", data_config.mixed_thinking)
        if data_config.mixed_thinking:
            print("success!")
            sampler = StatefulPairRandomSampler(data_source=dataset, generator=train_dataloader_generator)
        else:
